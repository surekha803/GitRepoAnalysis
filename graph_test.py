...
query_job = bqclient.query(""" select main_repo_id as repo_id,PartA_refrepos as partA_ref from github-project-310921.githubproject_rawdataset.PartA_ref_repos_final
 """).result().to_dataframe(bqstorage_client=bqstorageclient)
print(query_job.head())

G=nx.from_pandas_edgelist(query_job, 'repo_id', 'partA_ref', create_using=nx.DiGraph())
